...
function tetcommit()
	...
	rowcheck()
	if debug then printthetable() end
end

function turncc()
	if tchar ~= 'o' then
		local new = {
		{ blocks[4][1]+blocks[4][2]-blocks[1][2], blocks[1][1]+blocks[4][2]-blocks[4][1] },
	...
	local canturn = true
		for i=1,#new,1 do
			...
			if new[i][2] > #TileTable or new[i][1] > #TileTable[1] or new[i][2] < 1 or new[i][1] < 1 or TileTable[new[i][2]+2][new[i][1]+1] ~= ' ' then
				canturn = false
			end
		end
		if canturn then
...
function turnc()
	...
	if tchar ~= 'o' then
		local new = {
			{ blocks[1][2]+blocks[4][1]-blocks[4][2], blocks[4][1]+blocks[4][2]-blocks[1][1] },
...
function rowcheck()
	...
	for i=1,#TileTable-1,1 do
		...
		for j=1,#TileTable[i],1 do
			if TileTable[i][j] == ' ' then rowcomp = false end
		end
		if rowcomp then table.insert(columnstogo,i) end
	end
  ...
  if #columnstogo == 1 then score = score + 40*(level+1) end
  if #columnstogo == 2 then score = score + 100*(level+1) end
  if #columnstogo == 3 then score = score + 300*(level+1) end
  if #columnstogo == 4 then score = score + 1200*(level+1) end
end
...
function love.draw()
	...
	if state == "splash" then
		...
		love.graphics.printf("press start",0,love.graphics.getHeight()-64*scale,love.graphics.getWidth(),"center")
	end
	if state == "game" then
		if ghosting then ghostdraw() end
		tetdraw()
		love.graphics.setColor(0,0,0)
		...
		if paused then love.graphics.printf("PAUSED",0,28,love.graphics.getWidth(),"center") end
	end
	if state == "gameover" then
		love.graphics.setColor(0,0,0)
		love.graphics.rectangle("fill",0,0,love.graphics.getWidth(),32*scale)
...
function love.update(dt)
	if state == "game" and not paused then
		 if love.keyboard.isDown("down") or love.keyboard.isDown("kp2") then m = 5 else m = 1 end
		time = time+m*dt
...
function love.keypressed(key)
	...
	elseif state == "game" then
		if not paused then
			...
			if key == "up" or key == "lalt" then turncc() end
			if key == "down" or key == "kp2" then love.audio.play(move) end
			if key == " " then harddrop() end
			if key == "g" then
				if ghosting == true then ghosting = false else ghosting = true end
			end
		end
		if key == "p" then
			if paused == true then
    ...
    if key == "m" then
		...
		if key == "1" then level = 1 end
		if key == "2" then level = 2 end
		if key == "3" then level = 3 end
		if key == "4" then level = 4 end
		if key == "5" then level = 5 end
		if key == "6" then level = 6 end
		if key == "7" then level = 7 end
		if key == "8" then level = 8 end
		if key == "9" then level = 9 end
		if key == "0" then level = 30 end
